...
public class ImageProcessor implements Serializable {
    ...
    private static Mat getMat(VideoEventData ed) throws Exception {
        ...
        //System.out.println("333333" + ed.getData());
    ...
    public static VideoEventData process2(String camId, Iterator<VideoEventData> frames, VideoEventData previousProcessedEventData, DefaultTableModel model) throws Exception {
        VideoEventData currentProcessedEventData = new VideoEventData();
        Mat frame = null;
        double imageWidth = 640;
        double imageHeight = 480;
        ...
        for (VideoEventData eventData : sortedList) {
            ...
            if (frameCount == 10) {
                ...
                String match = ImageClassifier.classifyImage(bytes);
                logger.info("Best Match " + match);
                saveImageAndData(frame, eventData, match, model);
                frameCount = 0;
            }
            currentProcessedEventData = eventData;
        }
    ...
    private static void saveImageAndData(Mat mat, VideoEventData ed, String match, DefaultTableModel model) throws IOException {
        System.out.println("数据已经添加：" + ed.getCameraId());
    }
